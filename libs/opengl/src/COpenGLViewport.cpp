...
void COpenGLViewport::TRenderMatrices::applyLookAt()
{
...
void COpenGLViewport::renderNormalSceneMode() const
{
	...
	if (_.is_projective)
	{
		gluPerspective(_.FOV, vw / double(vh), m_clip_min, m_clip_max);
		CHECK_OPENGL_ERROR();
	}
	else
	{
		const double ratio = vw / double(vh);
		double Ax = _.eyeDistance * 0.5;
		double Ay = _.eyeDistance * 0.5;
